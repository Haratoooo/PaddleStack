  ...
  <div class="min-h-screen bg-white font-sans text-gray-800 pb-20">
    ...
    <main class="max-w-[1200px] mx-auto px-4 mt-10">
      ...
      <div v-else class="w-full overflow-x-auto pb-6">
        <div class="min-w-[800px] grid grid-cols-5 gap-x-6 gap-y-4 items-center px-2">
          ...
          <template v-for="time in times" :key="time">
            ...
            <div v-for="court in courts" :key="court" class="w-full">
              <button 
                ...
                class="w-full py-3.5 rounded-xl font-bold text-xs tracking-tight transition-transform active:scale-95 shadow-sm uppercase"
                :class="{
        ...
        <div class="md:hidden text-center mt-4 text-gray-400 text-xs font-medium animate-pulse">
          ← Swipe to view all courts →
        </div>
      </div>
